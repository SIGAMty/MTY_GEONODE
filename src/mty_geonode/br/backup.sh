...
# ##########################################################
# Run a backup
#  SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./mty_geonode/br/backup.sh $BKP_FOLDER_NAME
#   - BKP_FOLDER_NAME:
...
#
# e.g.:
#  docker exec -it django4mty_geonode sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./mty_geonode/br/backup.sh $BKP_FOLDER_NAME'
# ##########################################################
...
echo "-----------------------------------------------------"
echo "STARTING mty_geonode BACKUP $(date)"
echo "-----------------------------------------------------"
...
./manage.sh backup -i -f -c $PWD/mty_geonode/br/settings_docker.ini --backup-dir /$BKP_FOLDER_NAME/
